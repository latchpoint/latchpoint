...
- [ ] **AC-9:** `run_rules(triggering_entity_ids=["binary_sensor.back_door", "binary_sensor.front_door"])` for a rule whose `when` references both entities sets `ctx.triggers.trigger.entity_id == "binary_sensor.back_door"` (first in entity_id-sorted order — the dispatcher batch is a set; the engine sorts matched entity_ids for determinism) and `len(ctx.triggers.triggers) == 2`. *(Implemented; dedicated test TODO.)*
