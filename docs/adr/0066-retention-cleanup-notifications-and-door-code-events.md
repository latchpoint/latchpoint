...
Introduce UI-editable system config keys:

- `notifications.logs.retention_days` (default: 30)
- `notifications.deliveries.retention_days` (default: 30; applies to `sent`/`dead` only)
- `door_codes.events.retention_days` (default: 90)

Semantics:
- `<= 0` disables cleanup (no rows are deleted).
...
- Delete `NotificationLog` rows older than `notifications.logs.retention_days`.
- Delete `NotificationDelivery` rows in terminal states (`sent`, `dead`) older than `notifications.deliveries.retention_days`.
- Delete `DoorCodeEvent` rows older than `door_codes.events.retention_days`.

Tasks should:
- Run daily during the existing cleanup window (around the other 03:00–04:00 tasks).
...
- Add/confirm supporting indexes for efficient range deletes (e.g., `created_at` on notifications tables, `created_at` on `DoorCodeEvent` is already indexed indirectly via composite indexes).
