...
def _get_mqtt_settings() -> dict:
    ...
    if not isinstance(db_settings, dict):
        db_settings = {}
    # DB operational settings override env defaults
    return {
        "enabled": bool(db_settings.get("enabled", False)),
        "host": cfg["host"],
        "port": cfg["port"],
...
class MqttSettingsView(APIView):
    ...
    def patch(self, request):
        """Update MQTT settings (admin-only)."""
        data = request.data
        ...
        # Merge incoming fields into current DB settings
        if "enabled" in data:
        ...
        # Build the full merged settings for response and runtime
        settings = _get_mqtt_settings()
