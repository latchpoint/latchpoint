...
class RuleRuntimeStateTests(TestCase):
    ...
    def test_cooldown_active_false_without_cooldown(self):
        rule = Rule.objects.create(
            name="R", kind="trigger", enabled=True, priority=0, schema_version=1, definition={}, cooldown_seconds=None
        )
    ...
    def test_cooldown_active_false_without_last_fired(self):
        rule = Rule.objects.create(
            name="R", kind="trigger", enabled=True, priority=0, schema_version=1, definition={}, cooldown_seconds=60
        )
    ...
    def test_cooldown_active_true_when_recent(self):
        ...
        runtime = RuleRuntimeState.objects.create(
            rule=rule, node_id="when", status="pending", last_fired_at=now - timedelta(seconds=30)
        )
    ...
    def test_cooldown_active_false_when_elapsed(self):
        ...
        runtime = RuleRuntimeState.objects.create(
            rule=rule, node_id="when", status="pending", last_fired_at=now - timedelta(seconds=61)
        )
        self.assertFalse(cooldown_active(rule=rule, runtime=runtime, now=now))
