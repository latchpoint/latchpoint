...
from alarm.use_cases.alarm_actions import validate_user_code


class AlarmActionsValidateUserCodeTests(TestCase):
    def setUp(self):
    ...
    def test_maps_code_required_error(self, mock_validate):
        ...
        with self.assertRaises(sm_errors.CodeRequiredError) as ctx:
            validate_user_code(user=self.user, raw_code=None)
    ...
    def test_maps_invalid_code_error(self, mock_validate):
        ...
        with self.assertRaises(sm_errors.InvalidCodeError) as ctx:
            validate_user_code(user=self.user, raw_code="9999")
        self.assertEqual(str(ctx.exception), "Invalid code.")

    @patch("accounts.use_cases.code_validation.validate_user_code")
    @patch("alarm.use_cases.alarm_actions.timezone.now")
    def test_passes_timezone_now_through(self, mock_now, mock_validate):
        ...
        validate_user_code(user=self.user, raw_code="1234")
