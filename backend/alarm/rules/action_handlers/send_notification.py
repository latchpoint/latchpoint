...
def execute(action: dict[str, Any], ctx: ActionContext) -> tuple[dict[str, Any], str | None]:
    ...
    The executor handles ``delay_seconds`` generically (ADR-0094): deferred
    invocations enqueue via ``PendingAction`` and re-enter this handler at
    fire time with ``delay_seconds`` already stripped from the payload.
    """
