...
Per ADR-0094 §9 decision (a), `alarm_trigger` no longer accepts
`delay_seconds`. Existing rules that authored
`{type: "alarm_trigger", delay_seconds: N}` are rewritten in-place to
the composition::
...
action fires. Both halves use `alarm_set_state` so the migration produces
output that's valid under decision (a) (no `delay_seconds` on
`alarm_trigger`). The reverse migration collapses the pair back into the
    ...
    for rule in Rule.objects.all():
        ...
        while i < len(then):
            ...
            if (
                ...
                and nxt.get("type") == "alarm_set_state"
                and nxt.get("state") == "triggered"
                and isinstance(nxt.get("delay_seconds"), int)
                and not isinstance(nxt.get("delay_seconds"), bool)
                and nxt["delay_seconds"] > 0
            ):
                new_then.append({"type": "alarm_trigger", "delay_seconds": int(nxt["delay_seconds"])})
                changed = True
                i += 2
                continue
            new_then.append(action)
